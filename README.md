...
### v5 Now with Graphical interface, CUDA support, many languages, and voice blending!

![Audiblez GUI on MacOSX](./imgs/mac.png)

Audiblez generates `.m4b` audiobooks from regular `.epub` e-books,
using Kokoro's high-quality speech synthesis with support for voice blending.
...
This is a copy of the original repository. I have modified it with Grok Code Fast 1 to add the voice blending functionality and fix FFMPEG errors. All credit to the authors of these tools, not to me.
...
```
## Voice Blending (New Feature)
Audiblez now supports Kokoro TTS's official tensor-level voice blending for more dynamic audiobooks. This allows you to combine two voices with custom weights for unique narration styles. Blending is done at the tensor level for better performance compared to string-based selection.

# How to Use Voice Blending
Specify blended voices in the format: "voice1:weight,voice2:weight", where weights sum to 100.

Examples:
```
audiblez book.epub -v "af_bella:30,am_michael:70" # 30% Bella (female American English) + 70% Michael (male American English)
audiblez book.epub -v "bf_emma:60,bm_daniel:40" # 60% Emma (female British English) + 40% Daniel (male British English)
```
...
## Improved ffmpeg Conversion

Recent updates have fixed common ffmpeg errors during audiobook compilation, including codec parameter incompatibilities, stream mapping issues, or metadata tag conflicts. These were often due to improper handling of image/audio metadata when merging chapter WAV files into .m4b format.

What's Improved:

Enhanced stream handling and codec mapping for reliable conversion (e.g., avoiding "Tag text incompatible with output codec" errors).
Better error checking and fallback codecs (e.g., PCM for WAV concatenation).
Automatic cleanup of temporary files to prevent conflicts in subsequent runs.
If you encounter ffmpeg issues, ensure you're running the latest version (>=4.4) via ffmpeg -version. The script now uses optimized commands to avoid common pitfalls.
...
example:
  ...
  audiblez book.epub -v "af_bella:70,am_echo:30"
